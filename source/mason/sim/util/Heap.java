 ...
 */
/**
 * Implementations of Heap functions in Java. This code is derived from the
 * HeapSort example algorithm in <i>Introduction to algorithms</i> by Cormen,
 * Leiserson and Rivest. Intentionally very simple.
 *
...
public class Heap implements java.io.Serializable
{
    ...
    /**
     * Returns the key value of the current min element. Returns null if there
     * is no such element. Does not extract the element.
     */
    ...
    /**
     * Returns the current min element. Returns null if there is no such
     * element. Does not extract the element.
     */
    ...
    /**
     * Removes all key-equal minimum elements and adds them to a Bag, which is
     * then is returned. You may provide a Bag -- putInHere -- to be filled in.
     */
    ...
    /**
     * Removes the first minimum element and its key from the heap, and returns
     * the minimum element. Will return null if the heap is empty
     */
    ...
    /**
     * Produces a new heap which is the union of this heap with the other. The
     * original heaps are not modified. The new heap is returned.
     */
